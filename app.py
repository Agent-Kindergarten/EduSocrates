...
def get_chain_by_pdfs()->Chain:
    ...
    chain = ConversationalRetrievalChain.from_llm(retriever=retriever,llm=llm,memory=memory)
    return chain
...
def set_input_area():
    ...
    if len(st.session_state["pdf_docs"]) == 0:
        ...
        return
    if st.session_state["pdf_processed"] == False:
        st.info("Click on 'Process' first.")
        return
    user_question = st.text_input("Start your talk here:")
    if user_question and st.session_state["chain"] is not None and st.session_state["recently_cleared_chat_history"] is False:
...
def set_sidebar_about_project_module():
    with st.sidebar:
        st.header("About")
        st.write("This is a demo of EduSocrates, a chatbot that helps you review what you've learned.")
        st.write("This is a project of [Agent Kindergarten](https://github.com/Agent-Kindergarten).")
        st.write("You can find the source code [here](https://github.com/Agent-Kindergarten/EduSocrates)")
...
def set_sidebar_settings_module():
    with st.sidebar:
        ...
        st.text_input(label="- OpenAI API key",type="password",key="webpage_openai_api_key")
    ...
    with st.sidebar:
        if len(st.session_state["pdf_docs"]) == 0:
            ...
            if st.button(label="Process", disabled=False):
                ...
                st.success("✅ PDFs has been processed!")
